...
const NavItem = styled.li`
    ...
    a {
        ...
        padding: 8px 15px;
        border-radius: 5px;
        border-bottom: ${(props) => (props.isActive ? "2px solid rgb(0, 195, 255)" : "#333")};
...
const SideNavItem = styled(Link)`
    color: ${(props) => (props.isActive ? "linear-gradient(135deg, #02172d, #034874)" : "white")};
    font-weight: ${(props) => (props.isActive ? "700" : "500")};
    border-bottom: ${(props) => (props.isActive ? "1px solid rgb(0, 195, 255)" : "#333")};
    text-decoration: none;
    font-size: 16px;
    padding: 10px 15px;
    border-radius: 5px;
    ...
    return (
        <HeaderContainer>
            <Box>
                ...
                <NavIcon onClick={toggleNav}>{showNav ? <IoClose /> : <IoMenuOutline />}</NavIcon>
            </Box>
            ...
            <SideNav show={showNav}>
                ...
                {["/", "/Services", "/Portfolio", "/About", "/Contact"].map((path, index) => (
                    <SideNavItem key={index} to={path} isActive={activeNav === path} onClick={() => setShowNav(false)}>
                        {path === "/" ? "Home" : path.slice(1)}
                    </SideNavItem>
                ))}
                <Link to="/request_quote">
                    <QuoteButton onClick={() => setShowNav(false)}>Get a Quote</QuoteButton>
                </Link>
            </SideNav>
        </HeaderContainer>
    );
